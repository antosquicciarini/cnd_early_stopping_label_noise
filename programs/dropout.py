...
class ExampleTiedDropout(torch.nn.Module):
    ...
    def __init__(self, device,  p_fixed = 0.2, p_mem = 0.1, num_batches = 100, drop_mode = "train"):
        ...
        if self.drop_mode == "train":
            ...
            if epoch > 0:
                ...
                if self.mask_tensor is None:
                    #self.mask_tensor = torch.zeros(self.max_id, X.shape[1], X.shape[2], X.shape[3]).to(self.device)
                    self.mask_tensor = torch.zeros(self.max_id, X.shape[1]).to(self.device)
                #assign mask at positions given by idx
                self.mask_tensor[idx] = mask.to(self.device)
            
            # Apply the mask to the input tensor
            #X = X * mask.to(self.device)
            if len(X.shape) == 4: #Convolutional Layer
                mask_expanded = mask[:, :, None, None].expand(-1, -1, X.shape[2], X.shape[3])  # Now shape is [128, 64, 1, 1]
                X = X * mask_expanded.to(self.device)
            else:
                X = X * mask.to(self.device)
